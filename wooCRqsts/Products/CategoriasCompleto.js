...
module.exports = {
    GetcategoriasAll : function(req,res){
		return new Promise (resolve => {
            (async () => {
                            
                var pr_idEmpresa = req.query.pr_idEmpresa ;                
                connection.query('SELECT * FROM mv_categorias', function (error, results, fields) {
                    if (error)
                        throw error;
                
                        var jsonContent = JSON.stringify(results);                         
                        fs.writeFile("Categorias.json", jsonContent, 'utf8', function (err) {
                            if (err) {
